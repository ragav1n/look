...
export default function Wallet() {
  return (
    <div>
      <h1 className="font-display text-[26px] font-medium text-black">Wallet & Rewards</h1>
      ...
      <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <div className="rounded-card bg-card p-6">
          ...
          <p className="mt-2 text-[32px] leading-none font-medium text-black">{walletPoints}</p>
          <p className="mt-3 text-[13px] text-body">100 points = Rs. 100 off</p>
        </div>
      </div>

      <h2 className="mt-10 text-[16px] font-medium text-black">Transaction history</h2>
      <ul className="mt-4 flex flex-col divide-y divide-line border-y border-line">
        {walletTransactions.map((t) => (
          <li key={t.id} className="flex items-center justify-between py-4">
            <div>
              <p className="text-[15px] text-black">{t.description}</p>

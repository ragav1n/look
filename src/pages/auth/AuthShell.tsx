...
export const authInputClass =
  "h-[48px] w-full rounded-btn border border-line bg-surface px-4 text-[15px] text-white outline-none transition-colors placeholder:text-muted focus:border-accent";
...
export function GoogleButton({ onClick, label }: { onClick: () => void; label: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex h-[48px] w-full cursor-pointer items-center justify-center gap-3 rounded-btn border border-line bg-surface text-[15px] font-medium text-white transition-colors hover:border-line-strong focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 focus-visible:ring-offset-2 focus-visible:ring-offset-page"
    >
      {/* Monochrome Google "G" — keeps the strict black/red theme (no brand blue/green/yellow) */}
      <svg viewBox="0 0 48 48" width="20" height="20" aria-hidden fill="currentColor">
